        ...
        <div className={`flex flex-col h-full bg-white glass p-4 lg:p-6 w-full`}>
          ...
          <div className="mt-auto pt-6 border-t border-gray-100">
            {isAuthenticated ? (
              <div className="flex items-center justify-between px-1 lg:px-2">
                <div className="flex items-center gap-3">
                  ...
                  <div className={`flex flex-col hidden lg:block`}>
                    <span className="text-sm font-bold text-gray-900 truncate max-w-[120px]">{user.username}</span>
                  </div>
                </div>
              </div>
            ) : (
              <Link 
                to="/login"
                className="flex items-center gap-4 px-4 py-3.5 rounded-2xl text-gray-500 hover:bg-gray-50 hover:text-gray-900 transition-all"
              >
